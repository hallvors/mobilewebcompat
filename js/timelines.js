...
var testDataService = 'http://compatentomology.com.paas.allizom.org/';
...
/**
* This function maps regions to countries (by data in toplists.js)
* and adds a list of regions to the NAV
*/
var listsByRegion = {};
function regionPicker() {
  var nav = document.getElementsByTagName('nav')[0];
  for(var i=0; i<topLists.length; i++) {
    if(!(topLists[i].region in listsByRegion)) {
      listsByRegion[topLists[i].region] = [];
    }
    listsByRegion[topLists[i].region].push(topLists[i]);
  }
  for(var region in listsByRegion) {
    nav.appendChild(elm('a', region, {href:'#region:' + region}));
  }

  quickSearchInit();
}

document.addEventListener('DOMContentLoaded', regionPicker, false);

function listCountriesInRegion(region) {
  region = unescape(region);
  var nav = document.getElementsByTagName('nav')[0];
  var i, a;
  // remove any older sub-nav
  var childNav = document.getElementsByTagName('nav')[1];
  childNav.innerHTML = '';
  for(i = 0; i < topLists.length; i++) {
    if(topLists[i].region === region) {
      childNav.appendChild(elm('a', topLists[i].name, {href:'#list:' + topLists[i].id}));
    }
  }
}

function fillTables(listId) {
...
function lightbox(evt){
  ...
  var div = document.body.appendChild(elm('div', '', {'className':'floatingbox', 'id':'lightbox'}));
  var loadingElm = div.appendChild(elm('img','', {src:'/images/Vector_Loading_fallback.gif', className:'loading', alt:'loading...'}));
  div.appendChild(elm('img', '', {'src':url, 'alt':'Screenshot of site from recent automated test', onload:function(){loadingElm.parentNode.removeChild(loadingElm)}, onerror:function(){loadingElm.parentNode.removeChild(loadingElm); div.appendChild(elm('p', 'Image failed to load :-/'))}}));
  div.appendChild(elm('br'));
  div.appendChild(elm('a', 'close', {href:'#',onclick: function(){div.parentNode.removeChild(div);return false;}}));
  evt.preventDefault();
}

function regressionTable(){
  return; // Tests no longer run regularly, let's not show test results
  ...
  if(shifts.length){
    ...
    for(var i=0, a; i<shifts.length; i++){
      ...
      a = tr.appendChild(elm('th', '', {className:'title'})).appendChild(elm('a', bug +' '+(masterBugTable.bugs[bug]?masterBugTable.bugs[bug].summary:''), {href:masterBugTable.bugs[bug] ? masterBugTable.bugs[bug].link : ''} ));
      ...
      if(masterBugTable.bugs[bug] && masterBugTable.bugs[bug].link){
        var theHost = normalizeHostname(getHostname(masterBugTable.bugs[bug].url));
        /*if(theHost){
          var xhr = new XMLHttpRequest();
          xhr.responseType =  'json';
          xhr.open('GET', testDataService + 'data/' + theHost, true);
          xhr.onreadystatechange = (function(td){return function(){
            var data = this.response;
            if(!data)return;
            td.appendChild(elm('br'));
            if(data.screenshots && data.screenshots[0]){
              td.appendChild(elm('a', 'View screenshot', {href:data.screenshots[0].file, onclick:lightbox}));
            }
            td.parentElement.testData = data;
          }})(td);
          xhr.send(null);
        }*/
      }
...
document.addEventListener('keydown', function(e){
  if(e.keyCode === 27){
    var elms = document.getElementsByClassName('floatingbox');
    elms[0] && elms[0].parentNode.removeChild(elms[0]);
  }
}, false);
...
function showTestCode(evt){
  ...
  if(bugdata && bugdata[bug]){
    evt.preventDefault();
    var div = elm('div', 'To run the test, try pasting this code in the console after spoofing as '+ bugdata[bug].ua +' and loading ', {id:'testcodeviewer', 'className': 'floatingbox'});
    ...
    /* Check if ancestor tr has a .testData property, show test data if it has */
    var ancestor = evt.target.parentNode;
    while(ancestor.tagName !== 'TR' && ancestor.tagName !== 'BODY'){
      ancestor = ancestor.parentNode;
    }
    if(ancestor.testData){
      div.appendChild(elm('p', 'Test data:'))
      var pre2 = div.appendChild(elm('pre'));
      pre2.appendChild(document.createTextNode(JSON.stringify(ancestor.testData.test_data, null, 2)))
    }
...
function showListDetails(newHash, excludeUS){
  ...
  var table = detailsDiv.appendChild(elm('table'), '', {border:1, className:'list-master-table'});
  row = table.appendChild(document.createElement('tr'));
  row.appendChild(document.createElement('th')).appendChild(document.createTextNode('Category'));
  row.appendChild(document.createElement('th')).appendChild(document.createTextNode('Number of sites'));
  row.appendChild(document.createElement('th')).appendChild(document.createTextNode('Status'));
  row.lastChild.width="70%";
  for(var listnum=0, list; list=topLists[listnum]; listnum++){
    if(newHash && list.id === listId){
      constructListRows(table, list);
    }
  }
  table.appendChild(elm('tbody')).appendChild(elm('tr')).id = newHash;
...
function quickSearchInit(){
    ...
    var qs = p.appendChild(document.createElement('textarea'));
    qs.placeholder = 'Type domain name(s) or *.pattern..';
    ...
    function addDomainDetails(){
        ...
        masterBugTable.lists.custom = {id:'custom', name:'Custom list: ' + data.length + ' domains',counts:{}, data:data, timedata:[]}
    ...
    if(location.hash && location.hash.indexOf('list:custom:')>-1){
        ...
        qs.value = unescape(searchFromHash);
        uniQueue();
    }
}
...
function elm(tagname, text, properties){
  var e=document.createElement(tagname);
  if(text)e.appendChild(document.createTextNode(text));
  properties = properties || {};
  for(var prop in properties)e[prop] = properties[prop];
  return e;
}
...
function normalizeHostname(str){ // remove any www prefix
  var parts = str.split(/\./);
